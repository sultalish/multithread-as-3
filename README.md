...
The programs uses 4 servants as 4 running threads using Runnable extention for the Servant class. 

Servants responsible for taking presents from the bag(unordered) and add them to the chain, writing "thank you" cards, check for correctness of the tags in the chains.
...
## Proof of correctness
The programs uses 8 sensors as 8 running threads using Runnable extention for the TemperatureSensor class. 

Sensors collect temperatures. I use
```
temperature.set(rand.nextInt(171) - 100);
``` 
to collect temperature from -100F to 70F at every reading. 
